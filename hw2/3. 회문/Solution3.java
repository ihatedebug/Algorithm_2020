...
class Solution3 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			for(int i=1; i<n; i++) {	//base case		//n번 수행
				if (s.charAt(i-1) == s.charAt(i))
					DP[i-1][i]=2;
				else DP[i-1][i]=1;
			}

			for (int r=2; r<n; r++){		//n번 수행
				for(int i=0; i<n-r; i++){		//기껏해야 n번 수행
					int j=i+r;
			...
			//Complexity: theta(n^2)    : DP[i][j] = i번째부터 j 번째까지 문자열. i-1번째 문자와 j+1번째 문자 일치하는지 i<j+1에 대해 constant time 수행
			/////////////////////////////////////////////////////////////////////////////////////////////
