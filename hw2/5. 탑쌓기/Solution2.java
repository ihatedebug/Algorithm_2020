	...
	*/
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			 ...
			 */
			int table [][]={{1,1,0},{2,1,0},{0,2,2}};
			long DP[][][]= new long [n][n][3];
			for(int i=0; i<n; i++){		//base case // n번 수행
				DP[i][i][(int)s.charAt(i)-97]++;
			}
			for(int r=1; r<=n-1; r++){			//n-1번 수행
				for (int i=0; i<n-r; i++){		////기껏해야 n-1번 수행
					int j=i+r;
					for(int k=i; k<j; k++){		//기껏해야 n번 수행
						for(int p=0; p<3; p++){		//constant time
							for(int q=0; q<3; q++){		//constant time
								DP[i][j][table[p][q]]+=DP[i][k][p] * DP[k+1][j][q];
							}
						}
					}
			...
			//Time complexity: theta(n^3)   // DP[i][j]: i부터 j까지 곱했을 때 0,1,2 개수. i<=j에 대해서 최대 n번씩 수행
			/////////////////////////////////////////////////////////////////////////////////////////////
